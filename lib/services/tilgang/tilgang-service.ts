...
export const harTilgangTilBruker = cache(async (brukerIdent: string): Promise<boolean> => {
  if (mocksEnabled()) {
...
export const harTilgangTilBrukerPåSak = cache(async (saksId: string): Promise<boolean> => {
  const sak = await hentSak(saksId);
  if (sak == null) {
    return false;
  }

  return harTilgangTilBruker(sak.person.fodselsnummer);
});
